...
| 字段 | 类型 | 约束 |
| --- | --- | --- |
| `pageHeight` | 整数 | 1 ≤ pageHeight ≤ 10000；**正面**（第 1、3、5… 页）容量 |
| `backPageHeight` | 整数，可省略 | 1 ≤ backPageHeight ≤ 10000；**背面**（第 2、4、6… 页）容量。省略即单面模式，解析/计算/页面与导出结构与旧版逐项一致 |
| `blocks` | 数组 | 1 ≤ 长度 ≤ 200000，顺序即排版顺序 |
| `blocks[].id` | 字符串或数字 | 在数组内**唯一**（数字 `1` 与字符串 `"1"` 视为不同 id） |
| `blocks[].height` | 整数 | 单面：1 ≤ height ≤ pageHeight；双面：1 ≤ height ≤ max(pageHeight, backPageHeight)（高于某一面的块只会在放得下的面成页） |
...
- 点击「运行精确分页」后四秒内显示：**页数、代价 Σ(剩余高度)²、覆盖范围**（及每页
  块号/id 区间、面别、该面容量、已用/剩余高度）。
- **双面模式**：提供 `backPageHeight` 后，第 1 页为正面（容量 `pageHeight`），
  此后正反交替（容量 `backPageHeight`）；每页剩余高度按**该页实际面容量**计算，
  在所有满足边界约束且正背面交替的方案中最小化剩余平方和。同页链只落进放得下它
  的那一面；若奇偶位置导致任何一面都放不下则判无解。
...
**双面文件**额外固化：顶层 `backPageHeight`，以及每页的 `side`（`"front"`/`"back"`，
第 1 页正面、正反交替）与 `capacity`（该页实际容量）；重新导入即恢复双面计算语义。
单面文件一律不写这三个字段，结构与旧版完全一致。

## 优化模型与算法

令前缀高度和 `S[i]` 为前 i 块之和。

**单面**：页面 `(j,i)`（半开区间）代价 `(H − S[i] + S[j])²`，动态规划：

```
dp[i] = min over 合法 j: dp[j] + (H − S[i] + S[j])²
```

**双面**：第 1 页正面（Hf）、此后正反交替（Hb）。令 dpF[i]/dpB[i] 分别表示
「前 i 块排完、末页为正/背面」的最小代价，虚拟态 dpB[0]=0（0 页之后接正面）：
...
两种模式下，每个可行起点 j 都是一条直线（斜率 `−2·S[j]` 随 j 严格递减，查询点
`S[i]` 严格递增）：单面用一个、双面用**两个独立的凸包单调队列**（正面队列的线
来自 dpB、背面队列的线来自 dpF），均摊 O(n)、空间 O(n)。状态只按「末页面别」
二分，**不按页数展开**，也不是「先用单一容量分页再校验奇偶」。

实现要点（见 `src/core/paginate.ts` 注释）：

- 边界约束转化为页面**起点/终点可行性**：页内不得跨越强制分页；页首前边界、
  页尾边界不得是「同页」。「同页链总高 > 任一面容量」在线性预检中直接判无解；
  只适配一侧的链（放得下正面放不下背面、却被奇偶逼到背面）由 DP 兜底判无解。
...
- 双面两个队列各自维护截距（同一起点在两队列中截距不同）；交叉比较的斜率差
  系数 2 不可漏（漏写会偏保守地保留非凸中线，大高度差下失真，对拍锁定回归）。
...
- **小规模穷举（单面）**：n = 1..6、页容量 5/6/9、全部高度序列（单块高度 {1,2,3,5}）
  与全部边界赋值（n≤4 时含冲突的 4^(n−1) 全组合），逐例与朴素 O(n²) DP
  比较代价，并校验分页连续性、容量、全部边界约束与回算代价一致。
- **小规模穷举（双面）**：n = 1..6、(正面,背面) ∈ {(5,3),(4,4),(3,6)}、全部高度与
  边界赋值，逐例与**独立 O(n²) 页数奇偶 DP** 对拍，并校验面别奇偶、各面容量与
  同页边界。锁定验收例：正面 5 / 背面 3 / 块高 [2,3,2,3] → **三页、代价 5**
  （正面 [2,3] 满、背面 [2]、正面 [3]），而非单容量 H=5 的两个满页（代价 0）。
- **同页链只适配一侧**、强制分页下的交替、块高到两面较大值等手工例。
- **单面结构回归**：省略 `backPageHeight` 时模型不含该键，导出文件无
  `backPageHeight`、页面无 `side`/`capacity`，旧穷举与导出快照不变。
- **双面导出往返**：`backPageHeight` 与每页 `side`/`capacity` 固化，重新导入恢复
  双面语义；采纳快照不受后续编辑影响。
- **中等交叉验证**：5000 块单面、1200 块双面随机文档，分别与朴素 DP / 奇偶 DP 代价相等。
- **大夹具**：200,000 块单面两组随机种子、每页一块（20 万页、代价 0）、同页链无解；
  200,000 块双面（正 1000/背 700）与极端容量（正 10000/背 1）。原生运行 ≤ 4 s
  （实测约 0.1–0.2 s）。
